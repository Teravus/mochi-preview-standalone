...
def get_model_size(model_or_tensor):
    total_size = 0

    if isinstance(model_or_tensor, torch.nn.Module):
        # Handling case if it's a model
        for name, param in model_or_tensor.named_parameters():
            param_size = param.numel() * param.element_size()  # numel() gives number of elements, element_size() gives size in bytes
            total_size += param_size
            print(f"Layer: {name}, Size (bytes): {param_size}, Data type: {param.dtype}")

    elif isinstance(model_or_tensor, torch.Tensor):
        # Handling case if it's a tensor
        tensor_size = model_or_tensor.numel() * model_or_tensor.element_size()
        total_size += tensor_size
        print(f"Tensor Size (bytes): {tensor_size}, Data type: {model_or_tensor.dtype}")

    else:
        print("Error: Input must be a PyTorch model or tensor.")
        return
        ...
        if "gguf" in dit_checkpoint_path.lower():
            ...
            for name, param in model.named_parameters():
                if not any(keyword in name for keyword in params_to_keep):
                    ...
                    set_module_tensor_to_device(model, name, dtype=torch.bfloat16, device=self.device,
                                                value=dit_sd[name])
        else:
        ...
        if compile_args is not None:
            ...
            if compile_args["compile_dit"]:
                for i, block in enumerate(model.blocks):
                    model.blocks[i] = torch.compile(block, fullgraph=compile_args["fullgraph"],
                                                    dynamic=compile_args["dynamic"], backend=compile_args["backend"])
            if compile_args["compile_final_layer"]:
                model.final_layer = torch.compile(model.final_layer, fullgraph=compile_args["fullgraph"],
                                                  dynamic=compile_args["dynamic"], backend=compile_args["backend"])
        ...
        def model_fn(*, z, sigma, cfg_scale):
            ...
            if cfg_scale != 1.0:
                ...
                if self.fastercache_counter >= self.fastercache_start_step + 3 and self.fastercache_counter % 5 != 0:
                    ...
                    recovered_uncond = rearrange(recovered_uncond.to(out_cond.dtype), "(B T) C H W -> B C T H W", B=bb,
                                                 C=cc, T=tt, H=hh, W=ww)
